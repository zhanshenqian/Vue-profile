    ...
    <el-dialog :title="operate" v-model="dialogVisible" class='dialog'>
      <el-form  label-width="90px" :model='list' :rules="rules" ref='list'  >
        <el-form-item label="内容" prop='value'>
          <el-input v-model.trim='list.value' placeholder='请输入系统提示语' type="textarea" :autosize="{ minRows: 3, maxRows: 5}"></el-input>
        </el-form-item>
        <el-form-item>
...
<script>
  ...
  export default {
    data () {
      return {
        pages: {
          type: 2, // type 1 主播警告信息 2 系统消息 3关闭原因
          index: 1,
          size: 30
        },
        dialogVisible: false,
        ...
        rules: {
          value: [
            { required: true, message: '请输入系统提示语', trigger: 'change' }
          ]
        }
      }
    },
    methods: {
      ...
      edit (row) {
        let _this = this
        _this.dialogVisible = true
        _this.operate = '编辑系统提示语'
        _this.list = {value: row.Value, type: 2, gid: row.GID}
      },
      // 删除
      delet (row) {
        let _this = this
        _this.$confirm('确认新增系统提示语?', '提示', {
          type: 'warning'
        }).then(() => {
